  ...
  while(cont < N){
	...
	for(int nx = 0; nx < num1; ++nx){
	  ...
	  if(cont < N){
		...
		r[0] = (ssigma) * static_cast<double>(nx);
	      }else{
		r[0] = (ssigma) * (static_cast<double>(nx) + 0.5);
	      }
	      r[1] = (ssigma) * static_cast<double>(ny) * sq;
	    }else{
	      if(fmod(static_cast<double>(ny),2.) != 0){
		r[0] = (ssigma) * static_cast<double>(nx);
	      }else{
		r[0] = (ssigma) * (static_cast<double>(nx) + 0.5);
	      ...
	      r[1] = (ssigma) * (static_cast<double>(ny) * sq + sqt);
	    }
	    r[2] = (ssigma) * static_cast<double>(nz) * sqtt;
